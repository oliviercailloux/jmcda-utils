 ...
 * </p>
 * <p>
 * Implementation is based on 1) a sorted tree (or navigable set) containing the
 * keys and 2) a map (called base map) to associate values to the keys. The
 * sorted tree uses a key comparator. The keys comparator queries the base map
 * to get the values and handles the job to the user-provided entry comparator.
 * This object ensures that the base map contains the entries before the new
 * entries are added to the sorted tree. This is necessary for the key
 * comparator to be able to do the work.
 * </p>
 * <p>
 * The submap views provided by this object are only valid as long as the
 * existing ordering at time the map view is queried for is unchanged. Results
 * are undetermined if the ordering changes after the view has been created.
 * Changing ordering is normally not possible in a navigable map as the ordering
 * only depends on the key. Here, changing the ordering is possible through
 * setting a different value to an existing key. This results in the same key
 * possibly being ordered differently than previously, hence its ceiling key,
 * and so on, may change. Doing this is permitted by this object, but such
 * operations invalidate the submap views obtained previously.
 * </p>
 * <p>
 * The views do not fully support removal (not implemented yet).
 * </p>
 * <p>
 * The comparator used by this navigable map is extentional: it defines an order
 * only on the objects known to this map (this is a partial order on the set of
 * keys possibly accepted by this map). It may not be used to compare objects
...
public class KeyValueOrderedMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V> {
	...
	/**
	 * The base map always contains all keys entries from the tree. It may contain
	 * more keys than the tree (this happens when this object is used to provide a
	 * submap view).
	 */
	...
	public boolean containsValue(Object value) {
		...
		// }
		/**
		 * We have to search for the value because the base map may contain more values
		 * than are effectively in this object, because this object could be a partial
		 * view over a part.
		 */
	...
	public boolean containsKey(Object key) {
		...
		/**
		 * Need to check this, because the key could be in the base map and yet not be
		 * in this object.
		 */
	...
	/**
	 * <p>
	 * Creates a new navigable map which initially contains the entries contained in
	 * the given base map.
	 * </p>
	 * <p>
	...
	public V put(K key, V value) {
		...
		// }
		/**
		 * If the value changed, the key position in the ordering may change. Removing
		 * the key from the tree makes sure the key is correctly inserted.
		 */
	...
	public NavigableSet<K> descendingKeySet() {
		...
		/**
		 * We should observe this to sync removal operations with the base map, but not
		 * implemented yet.
		 */
	...
	public NavigableMap<K, V> subMap(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive) {
		...
		return new KeyValueOrderedMap<K, V>(m_comparator, m_baseMap, subSet) {
			...
			public V put(K key, V value) {
				...
				/**
				 * A check on not too much change (this is incomplete check but it can be done
				 * quickly).
				 */
