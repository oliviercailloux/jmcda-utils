...
/**
 * A set view that wraps an other set. When the user calls {#remove(Object)} on
 * this view, if the underlying set contains the object, this view calls first
 * {@link #beforeRemove(Object)}, then {@link #justRemove(Object)}, then
 * {@link #afterRemove(Object)}. Extenders of this view may override the methods
 * to do appropriate job before or after removal. When an iterator is asked on
 * this view, and the iterator remove method is used, this view first calls
 * {@link #beforeRemove(Object)}, then removes the object using the iterator
 * method, then calls {@link #afterRemove(Object)}.
 * 
 * @author Olivier Cailloux
 * 
 * @param <T> the type of the objects in the set view and the underlying set.
 */
public abstract class AbstractSetView<T> extends AbstractSet<T> implements Set<T> {
	...
	/**
	 * @param object has just been removed per {@link #justRemove(Object)} or per
