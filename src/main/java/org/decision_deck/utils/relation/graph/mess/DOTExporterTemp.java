...
/**
 * Temporary copy of the {@link org.jgrapht.ext.DOTExporter} class to overcome a bug.
 * 
 * @param <V>
 *            v
 * @param <E>
...
public class DOTExporterTemp<V, E> {
    ...
    /**
     * Constructs a new DOTExporter object with an integer name provider for the vertex IDs and null providers for the
     ...
     *            for generating vertex labels. If null, vertex labels will not be written to the file.
    ...
    /**
     * Constructs a new DOTExporter object with the given ID, label, and attribute providers. Note that if a label
     * provider conflicts with a label-supplying attribute provider, the label provider is given precedence.
     * 
     * @param vertexIDProvider
     *            for generating vertex IDs. Must not be null.
     * @param vertexLabelProvider
     *            for generating vertex labels. If null, vertex labels will not be written to the file (unless an
     *            attribute provider is supplied which also supplies labels).
     * @param edgeLabelProvider
     *            for generating edge labels. If null, edge labels will not be written to the file.
     * @param vertexAttributeProvider
     *            for generating vertex attributes. If null, vertex attributes will not be written to the file.
     * @param edgeAttributeProvider
     *            for generating edge attributes. If null, edge attributes will not be written to the file.
     */
    public DOTExporterTemp(VertexNameProvider<V> vertexIDProvider, VertexNameProvider<V> vertexLabelProvider,
     ...
     * http://www.graphviz.org/doc/info/lang.html Quoted from above mentioned source: An ID is valid if it meets one of
