...
/**
 * <p>
 * A matrix which is able to store values in [0, 1], i.e., doubles between 0 and
 * 1 inclusive, at a set of positions, each position being represented by a row
 * and a column.
 * </p>
 * <p>
 * Vocabulary note: an entry can be a value or can be <code>null</code>, a value
 * is a double between zero and one.
 * </p>
 * 
...
public interface SparseMatrixFuzzy<R, C> extends SparseMatrixFuzzyRead<R, C>, SparseMatrixD<R, C> {
	 ...
	 * @param value  between zero and one (inclusive).
	 */
	@Override
