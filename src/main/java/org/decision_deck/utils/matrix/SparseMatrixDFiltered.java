...
public class SparseMatrixDFiltered<R, C> implements SparseMatrixDRead<R, C> {
	...
	if (m_columnPredicate == null) {
	    ...
	    /**
	     * As we filter on columns as well, it might be that a row which passes the given predicate on rows has no
	     * evaluations on the remaining columns, and hence should not appear as a row of this object. Thus we need
	     * to make sure that the row, even if it passes the predicate, has at least one value, considering the
	     * filtered column.
	     */
