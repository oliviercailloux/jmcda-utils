...
/**
 * 
 * Decorates an other Set and adds an external ordering capability. This object is in one of three possible states.
 * <ul>
 * <li>uses delegate order;</li>
 * <li>uses a general comparator;</li>
 * <li>uses a {@link ExtensionalComparator} or a collection defining the order, in which case this object is said to be
 * in <em>subset mode</em>. In this mode, supplementary caution is required as adding an object to this set requires its
 * order to be defined.</li>
 * </ul>
 * 
 * @author Olivier Cailloux
 * 
 * @param <E>
 *            the type of elements stored in this set and in the delegated set.
 */
public class OrderDecoratedSet<E> extends ForwardingSet<E> implements Set<E> {
    /**
     * Creates a new decorator for the given set. The default is to use the order given by the delegate. Therefore the
     * returned set has the same iteration order as the given decorated set, to begin with.
     * 
     * @param <E>
    ...
    /**
     * May be <code>null</code>. If not <code>null</code>, the general comparator to use to order, defining the ordered
     * set.
     */
    private Comparator<? super E> m_comparator;
    /**
     * May be <code>null</code>. If not <code>null</code>, the subset comparator to use to order, defining the ordered
     * set.
     */
    private ExtensionalComparator<E> m_subsetComparator;
    /**
     * Is <code>null</code> iff no general comparator is defined. If not <code>null</code>, defines the ordering shown
     * by this object.
     */
    private NavigableSet<E> m_orderedSetGeneral;
    /**
     * Is <code>null</code> iff no subset comparator is defined. If not <code>null</code>, defines the ordering shown by
     * this object.
     */
     ...
     * <tt>e</tt> to this set if the set contains no element <tt>e2</tt> such that
     ...
     * </p>
     * <p>
     * This set implementation adds a supplementary restriction to adding an object: its order must be defined. Ordering
     * information must be given with any new element added as this object does not know otherwise how to treat them
     * compared to the other elements. Please use rather {@link #addAsBest(Object)} or {@link #addAfter(Object, Object)}
     * if order of the object to add is not known already. This restriction applies only if this object is in subset
     * ordering mode (see {@link OrderDecoratedSet}), it does not apply if it uses the order of the delegate or a
     * general comparator.
     * </p>
     * 
     * @throws UnsupportedOperationException
     *             if the given element has no known order.
     */
    @Override
    ...
    /**
     * Switches this object to delegate mode. Any previously set ordering information is lost.
     */
    ...
    /**
     * Switches this object to comparator mode. Any previously set ordering information is lost.
     * 
     * @param comparator
     *            not <code>null</code>, must be a general comparator (i.e. not a {@link ExtensionalComparator}).
     */
    ...
    /**
     * Retrieves a writable view to the decorated set underlying this object. The user of this object is reponsible for
     * ensuring the order of all objects in the decorated set is known to this object, otherwise the behavior of this
     * object is not determined any more. Thus it is not advised to add elements to the decorated set through this
     * method, rather add elements through this object's #add or similar methods which will ensure ordering information
     * is consistent. This method may be useful however to remove informations from the underlying decorated set without
     * modifying the ordering informations, which is especially useful in case the ordering information is shared.
     * 
     * @return not <code>null</code>.
     */
    ...
    /**
     * The facade this object finally shows. It delegates either to this object's delegate, that is, the decorated set,
     * or to the navigable set if defined. This view is a writeable view thus it manages the sync between this object
