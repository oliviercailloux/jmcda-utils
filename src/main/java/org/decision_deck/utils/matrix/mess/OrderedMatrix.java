...
/**
 * A decorator around an other matrix, which adds ordering capability to the decorated matrix. Two use cases are
 * possible for this object. The simplest one is to avoid sharing the references to its underlying matrix and orders and
 * let this object manage the adequacy between the ordering and the matrix. A second use case is possible to allow
 * sharing references. In that case, the user must always ensure that the rows and columns order contain all rows and
 * columns used in the matrix underlying this object. The contracts for the various methods suppose the user fulfills
 * this condition, otherwise this object behavior is unspecified.
 * 
...
public class OrderedMatrix<R, C> extends FloatMatrixForwarder<R, C> implements SparseMatrixD<R, C> {
    ...
    /**
     * Must contain all rows in the matrix. May contain rows that are not in the matrix.
     */
    ...
    /**
     * <p>
     * Decorates a matrix by adding ordering capability. By default, the iteration order of the delegate is used.
     * </p>
     * 
     * @param delegate
     *            not <code>null</code>.
    ...
    /**
     * These references may be shared, in which case the user of this object must make sure the row and column orders
     * always contain all rows and columns used in the given matrix.
     * 
     * @param rowOrder
     *            not <code>null</code>.
     */
    ...
    /**
     * Retrieves a view of the column order used by this object. Changing the returned order changes the order this
     * object uses and should thus be done with caution.
     * 
     ...
     * {@inheritDoc}
     * <p>
     * This method removes the information from the matrix <em>and</em> from the row or column ordering if the object
     * was the last in a given row or column. Adding the object back later will thus require to specify its position
     * again.
     * </p>
     */
    @Override
