...
/**
 * Various utility methods dealing with matrices, or as this class say, “matrixes”. The alternative plural has been
 * chosen because it allows this class to be found when searching for the word “Matrix”.
 * 
...
public class Matrixes {
    ...
    /**
     * Returns a new matrix ensuring every element it contains satisfies the given predicate.
     * 
    ...
    /**
     * Two matrixes are “approximately equal” to a given degree of precision iff they contain values for the same
     * mappings and the value they contain for each mapping are not more different than the given allowed imprecision.
     * 
     ...
     *            may be <code>null</code>.
     * @param imprecision
     *            a number positive or nul, or positive infinity.
     * @return <code>true</code> iff the given matrix is approximately equal to this one.
     */
    static public <R, C> boolean approxEqual(SparseMatrixDRead<R, C> m1, SparseMatrixDRead<R, C> m2, double imprecision) {
    ...
    /**
     * Retrieves a read-only view of the given delegate. The returned object is immutable iff the source is immutable.
     * 
     * @param <R>
     *            the row type.
     * @param <C>
	...
	// target.put(cell.getRowKey(), cell.getColumnKey(), cell.getValue().doubleValue());
	// }
    ...
    /**
     * (Old implementation, probably to delete.) Does not use the matrixes definition of approxEquals, so that this
     * method may be used in implementing objects.
     * 
     * @param <Row>
     *            the row type.
     * @param <Column>
     ...
     *            not <code>null</code>.
     * @param m2
     *            may be <code>null</code> (then returns <code>false</code>).
     * @param imprecision
     *            the allowed imprecision.
     * @return <code>true</code> iff both contents have values at the same positions and these values are not further
    ...
    /**
     * <p>
     * Retrieves the set of rows which exist in both matrices but are discording. Two rows are discording iff they do
     * not have the same set of column and value mapping.
     * </p>
     * <p>
     * As a consequence of this definition, if the sets of rows are disjoint, this method returns an empty set.
     * </p>
     * 
     * @param matrix1
     *            not <code>null</code>.
    ...
    /**
     * Checks that the matrix is such that m(a, b) + m(b, a) <= maxValue. The matrix must be complete and square.
     * 
     * @param <T>
     *            the type of rows and columns.
     * @param matrix
     *            not <code>null</code>.
     * @param maxValue
     *            infinite values are allowed.
     * @return <code>true</code> iff there exists a pair (a, b) such that the entry at (a, b) plus the value at (b, a)
