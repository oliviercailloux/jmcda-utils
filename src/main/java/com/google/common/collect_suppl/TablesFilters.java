...
/**
 * <p>
 * Own copy of a patch from http://codereview.appspot.com/4425056/, not integrated to Guava yet.
 * </p>
...
public final class TablesFilters {
    ...
    /**
     * Creates a transposed view of a given table that flips its row and column keys. In other words, calling
     * {@code get(columnKey, rowKey)} on the generated table always returns the same value as calling
     * {@code get(rowKey, columnKey)} on the original table. Updating the original table changes the contents of the
     * transposed table and vice versa.
     * 
     * <p>
     * The returned table supports update operations as long as the input table supports the analogous operation with
     * swapped rows and columns. For example, in a {@link HashBasedTable} instance, {@code rowKeySet().iterator()}
     * supports {@code remove()} but {@code columnKeySet().iterator()} doesn't. With a transposed {@link HashBasedTable}
	...
	class CellSet extends TransformedCollection<Cell<R, C, V>, Cell<C, R, V>> implements Set<Cell<C, R, V>> {
		...
		if (obj instanceof Cell) {
		    ...
		    return original.cellSet().contains(
			    immutableCell(cell.getColumnKey(), cell.getRowKey(), cell.getValue()));
		}
		return false;
	    }
    ...
    /**
     * Returns a table containing the cells in {@code unfiltered} whose rows satisfy a predicate. The returned table is
     * a live view of {@code unfiltered}; changes to one affect the other.
     * 
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered table or its views, only
     * cells whose rows satisfy the filter will be removed from the underlying table.
     * 
     * <p>
     * The returned table isn't threadsafe or serializable, even if {@code unfiltered} is.
     * 
     * <p>
     * Many of the filtered table's methods, such as {@code size()}, iterate across every cell in the underlying table
     * and determine which satisfy the filter. When a live view is <i>not</i> needed, it may be faster to copy the
     * filtered table and use the copy.
     * 
     * <p>
     * <b>Warning:</b> {@code rowPredicate} must be <i>consistent with equals</i>, as documented at
     * {@link Predicate#apply}. Do not provide a predicate such as {@code Predicates.instanceOf(ArrayList.class)}, which
     * is inconsistent with equals.
     */
    static public <R, C, V> Table<R, C, V> filterRows(Table<R, C, V> unfiltered, final Predicate<? super R> rowPredicate) {
    ...
    /**
     * Returns a table containing the cells in {@code unfiltered} whose columns satisfy a predicate. The returned table
     * is a live view of {@code unfiltered}; changes to one affect the other.
     * 
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered table or its views, only
     * cells whose columns satisfy the filter will be removed from the underlying table.
     * 
     * <p>
     * The returned table isn't threadsafe or serializable, even if {@code unfiltered} is.
     * 
     ...
     * 
     * <p>
     * <b>Warning:</b> {@code columnPredicate} must be <i>consistent with equals</i>, as documented at
     * {@link Predicate#apply}. Do not provide a predicate such as {@code Predicates.instanceOf(ArrayList.class)}, which
     * is inconsistent with equals.
     */
    ...
    /**
     * Returns a table containing the cells in {@code unfiltered} whose values satisfy a predicate. The returned table
     * is a live view of {@code unfiltered}; changes to one affect the other.
     * 
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered table or its views, only
     * cells whose values satisfy the filter will be removed from the underlying table.
     * 
     * <p>
     * The returned table isn't threadsafe or serializable, even if {@code unfiltered} is.
     * 
     ...
     * 
     * <p>
     * <b>Warning:</b> {@code valuePredicate} must be <i>consistent with equals</i>, as documented at
     * {@link Predicate#apply}. Do not provide a predicate such as {@code Predicates.instanceOf(ArrayList.class)}, which
     * is inconsistent with equals.
     */
    ...
    /**
     * Returns a table containing the cells in {@code unfiltered} which satisfy a predicate. The returned table is a
     * live view of {@code unfiltered}; changes to one affect the other.
     * 
     * <p>
     * When methods such as {@code removeAll()} and {@code clear()} are called on the filtered table or its views, only
     * cells which satisfy the filter will be removed from the underlying table.
     * 
     * <p>
     * The returned table isn't threadsafe or serializable, even if {@code unfiltered} is.
     * 
     ...
     * 
     * <p>
     * <b>Warning:</b> {@code cellPredicate} must be <i>consistent with equals</i>, as documented at
     * {@link Predicate#apply}. Do not provide a predicate such as {@code Predicates.instanceOf(ArrayList.class)}, which
     * is inconsistent with equals.
     */
    ...
    /**
     * Returns a view of a table where each value is transformed by a function. All other properties of the table, such
     * as iteration order, are left intact.
     * 
     * <p>
     * Changes in the underlying table are reflected in this view. Conversely, this view supports removal operations,
     * and these are reflected in the underlying table.
     * 
     * <p>
     * It's acceptable for the underlying table to contain null keys, and even null values provided that the function is
     * capable of accepting null input. The transformed table might contain null values, if the function sometimes gives
     * a null result.
     * 
     * <p>
     ...
     * 
     * <p>
     * The function is applied lazily, invoked when needed. This is necessary for the returned table to be a view, but
     * it means that the function will be applied many times for bulk operations like {@link Table#containsValue} and
     * {@code Table.toString()}. For this to perform well, {@code function} should be fast. To avoid lazy evaluation
     * when the returned table doesn't need to be a view, copy the returned table into a new table of your choosing.
     * 
